...
export default function Hooksmith() {
  ...
  useEffect(() => {
    try { const p = localStorage.getItem("hooksmith-my-prompts"); if (p) setMyPrompts(JSON.parse(p)); } catch {}
    try { const h = localStorage.getItem("hooksmith-history"); if (h) setHistory(JSON.parse(h)); } catch {}
    setStorageReady(true);
  }, []);

  useEffect(() => {
    if (!storageReady) return;
    try { localStorage.setItem("hooksmith-my-prompts", JSON.stringify(myPrompts)); } catch {}
  }, [myPrompts, storageReady]);

  useEffect(() => {
    if (!storageReady) return;
    try { localStorage.setItem("hooksmith-history", JSON.stringify(history)); } catch {}
